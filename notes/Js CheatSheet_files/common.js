...
const LANGUAGES = new Map([
    ...
    const checkMarkIcon = `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#7C3AED" class="size-6" width=20 style="display:inline">
...
function paginate(tag, title, pages, pageDirectives, pagination, tableOfContents) {
    ...
    const isRtl = LANGUAGES.get(selectedLanguageCode).rtl;
    ...
    const previousButtonHtml = isRtl ? `<button class="previous">${arrowRight}</button>`
                                     : `<button class="previous">${arrowLeft}</button>`;
    const nextButtonHtml = isRtl ? `<button class="next">${arrowLeft}</button>`
                                 : `<button class="next">${arrowRight}</button>`;

    const progressBar = document.querySelector('nav.instruction-progress-bar');
    const progressFill = document.querySelector('div.instruction-progress-fill');
    if (isRtl) {
        progressBar.style.direction = 'rtl';
        progressFill.style.left = 'unset'; // Reset left
        progressFill.style.right = '0'; //
    } else {
        // Apply LTR styles
        progressBar.style.direction = 'ltr';
        progressFill.style.right = 'unset'; // Reset right
        progressFill.style.left = '0'; // Fill starts from the left
    }
...
function setPreviewWatchToolbar(doc) {
    ...
    // populate toolbar
    toolbar.append(`<button class="toc tool-icon">${tableOfContentsIcon}</button>`);
        ...
        fetch(localesUrl)
            ...
            .then(localesData => {
                const jsonData = {
                    ...
                    locales: Object.entries(localesData.locales).map(([language, data]) => ({
                        language,
                        url: data.url
                    }))
                };

                let selectedLanguageCode = localStorage.getItem('selectedLanguageCode') || jsonData.primary_locale;
                const match = jwt.md_instructions_url.match(/-([a-z]{2})\.md$/i);
                const currentLanguageCode = match ? match[1].toUpperCase() : jsonData.primary_locale;

                console.log(selectedLanguageCode, currentLanguageCode)
                console.log(!window.location.href.includes('edit') && selectedLanguageCode !== currentLanguageCode)

                if (!window.location.href.includes('edit') && selectedLanguageCode !== currentLanguageCode) {
                    window.location.replace(jsonData.locales.find(locale => locale.language === selectedLanguageCode).url + '.html');
                }

                if (!selectedLanguageCode || !LANGUAGES.get(localStorage.getItem('selectedLanguageCode'))) {
                    localStorage.setItem('selectedLanguageCode', jsonData.primary_locale);
                    selectedLanguageCode = jsonData.primary_locale;
                }

                if (LANGUAGES.get(selectedLanguageCode).rtl) {
                    const previewContainer = document.querySelector('.editormd-preview-container');
                    previewContainer.classList.add('rtl');
                }
                ...
                if (jsonData) {
                    ...
                    for (const locale of jsonData.locales) {
                        ...
                        toolbar.find('#translations-dropdown').append(`
                            <a href="${locale.url}.html" class="dropdown-translation-item-block" data-lang="${locale.language}">
                                <span>${LANGUAGES.get(locale.language).native} ${locale.language === jsonData.primary_locale ? '[Original]' : ''}</span>
                                ${locale.language === selectedLanguageCode ? `<span>${checkMarkIcon}</span>` : ''}
                            </a>
                        `);
                    }
                    ...
                    toolbar.find('#translations-dropdown').find('a').bind(editormd.mouseOrTouch("click", "touchend"), function (event) {
